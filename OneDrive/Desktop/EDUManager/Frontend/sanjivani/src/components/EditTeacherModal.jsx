...
function EditTeacherModal({ open, onClose, onSubmit, loading, error, form, onChange }) {
  const [departments, setDepartments] = useState([]);
  const [allSubjects, setAllSubjects] = useState([]);
  const [selectedSubjects, setSelectedSubjects] = useState(form.subjects || []);

  useEffect(() => {
    if (open) {
      getDepartments().then(setDepartments);
      getSubjects().then(setAllSubjects);
      setSelectedSubjects(form.subjects || []);
    }
  }, [open, form.subjects]);
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50 transition-opacity">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md relative animate-fade-in">
        ...
        <form onSubmit={e => onSubmit(e, selectedSubjects)} className="space-y-4">
          ...
          <button type="submit" className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white px-4 py-2 rounded w-full font-semibold hover:scale-105 transition-transform" disabled={loading}>{loading ? 'Saving...' : 'Save Changes'}</button>
        </form>
      </div>
    </div>
  );
